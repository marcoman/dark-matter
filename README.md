...
This repository contains **four implementations** of the same web application:
...
| [`cpp/`](cpp/) | C++ (Crow), CMake, Docker |

All implementations expose the same routes and product behavior: name-only login, four-corner compass navigation, optional About page, banner styling and experiments driven by **LaunchDarkly** feature flags, and browser-reported metrics for experiments (e.g. nav theme, inline About load time).
...
- **[`cpp/README.md`](cpp/README.md)** — Ubuntu packages (apt), CMake build, Docker image under `cpp/`.
